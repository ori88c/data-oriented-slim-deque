...
const data_oriented_slim_deque_1 = require("./data-oriented-slim-deque");
function getNewCapacity(oldCapacity, incrementFactor) {
 ...
 */
function fillWithItems(numberOfItems, capacityIncrementFactor, deque) {
    ...
    let expectedSize = 0;
    const pushAndCheckCapacity = (pushOperation) => {
...
function popAllItems(deque) {
    ...
    const popAndValidate = (popOperation) => {
        popOperation();
        --expectedSize;
...
function pushAllPopAllTest(initialCapacity, capacityIncrementFactor, numberOfItems) {
    const deque = new data_oriented_slim_deque_1.SlimDeque(initialCapacity, capacityIncrementFactor);
...
describe('SlimDeque tests', () => {
    describe('Happy path tests', () => {
        test('push all items in a randomized order, then pop all items in a randomized order, ' +
        ...
        test('push items in a randomized order, clear the deque, validate successful clearing, and repeat', async () => {
            ...
            const deque = new data_oriented_slim_deque_1.SlimDeque();
            for (let currRepetition = 0; currRepetition < repetitionsCount; ++currRepetition) {
                // Push all items in ascending front-to-back order: 1, 2, ..., numberOfItems.
                fillWithItems(numberOfItems, data_oriented_slim_deque_1.DEFAULT_SLIM_DEQUE_CAPACITY_INCREMENT_FACTOR, deque);
                deque.clear();
                expect(deque.size).toBe(0);
                expect(deque.isEmpty).toBe(true);
            }
        });
        test('push a random number of items to random ends (randomly choosing front or back for each push), ' +
            ...
            const deque = new data_oriented_slim_deque_1.SlimDeque(); // Default params (initial capacity, capacity increment factor).
            const pushToRandomEnd = () => {
    ...
    describe('Negative path tests', () => {
        test('constructor should throw when the initial capacity is not a natural number', () => {
            ...
            for (const invalidInitialCapacity of nonNaturalNumbers) {
                expect(() => new data_oriented_slim_deque_1.SlimDeque(invalidInitialCapacity)).toThrow();
            }
        });
        test('constructor should throw when the capacity increment factor is too small', () => {
            ...
            for (const factor of tooSmallIncrementFactor) {
                expect(() => new data_oriented_slim_deque_1.SlimDeque(validInitialCapacity, factor)).toThrow();
            }
        });
        ...
        test('should throw an error when accessing the front and back getters on an empty instance', () => {
            ...
            for (let currentRepetition = 1; currentRepetition <= numberOfRepetitions; ++currentRepetition) {
                ...
                expect(deque.size).toBe(0);
                expect(deque.isEmpty).toBe(true);
                expect(deque.capacity).toBe(data_oriented_slim_deque_1.DEFAULT_SLIM_DEQUE_INITIAL_CAPACITY);
            }
        });
    });
});
//# sourceMappingURL=data-oriented-slim-deque.test.js.map